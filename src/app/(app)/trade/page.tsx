        ...
        <div className="grid grid-cols-[380px_1fr] gap-5 items-stretch">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 h-full flex flex-col">
            ...
            <div className="flex rounded-lg border border-slate-200 overflow-hidden mb-4">
              <button
                onClick={() => setMode('buy')}
                className={`flex-1 py-2 text-sm font-bold transition-colors ${mode === 'buy' ? 'bg-emerald-500 text-white' : 'text-slate-500 hover:bg-slate-50'}`}
              >Buy</button>
              <button
                onClick={() => setMode('sell')}
                className={`flex-1 py-2 text-sm font-bold transition-colors ${mode === 'sell' ? 'bg-red-500 text-white' : 'text-slate-500 hover:bg-slate-50'}`}
              >Sell</button>
            </div>
                  ...
                  <>
                      ...
                      <div className="border border-slate-200 rounded-lg overflow-hidden max-h-44 overflow-y-auto">
                        {holdings.map(h => (
                          <div key={h.ticker}
                            ...
                            className={`flex items-center gap-3 px-3 py-2.5 cursor-pointer hover:bg-slate-50 border-b border-slate-100 last:border-0 ${selectedHolding?.ticker === h.ticker ? 'bg-indigo-50' : ''}`}
                          >
                            <span className="font-bold text-slate-800 text-sm w-14">{h.ticker}</span>
                            <span className="flex-1 text-xs text-slate-400">{h.quantity} share{h.quantity !== 1 ? 's' : ''}</span>
                            <span className="text-xs font-semibold text-slate-600">{fmt(h.current_price)}</span>
                    ...
                    {selectedHolding && (
                      <>
                        <div className="flex justify-between items-center bg-slate-50 rounded-lg px-3 py-2 mb-3 border border-slate-200">
                          <div>
                            <span className="font-extrabold text-slate-800 text-sm">{selectedHolding.ticker}</span>
                            <span className="text-xs text-slate-400 ml-2">{selectedHolding.quantity} shares owned</span>
                          </div>
                          <span className="font-bold text-slate-800 text-sm">{fmt(selectedHolding.current_price)}</span>
                        </div>
                        ...
                        <div className="mb-2">
                          ...
                          <input type="number" min="1" max={selectedHolding.quantity} step="1" value={sellShares}
                            ...
                            className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-red-400 transition-colors" />
                        </div>
                        ...
                        {parseFloat(sellShares) > 0 && (
                          <div className="bg-slate-50 rounded-xl px-3 py-2.5 mb-3 border border-slate-100">
                            <div className="flex justify-between text-sm font-bold py-1"><span>Proceeds</span><span>{fmt(parseFloat(sellShares) * selectedHolding.current_price)}</span></div>
                            <div className="flex justify-between text-xs">
                              <span className="text-slate-400">Cash after trade</span>
