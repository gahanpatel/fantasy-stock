...
export default function TradePage() {
  ...
  function setQuickQty(frac: number) {
    ...
    if (side === 'buy') {
      ...
      const held = holdings.find(h => h.ticker === selected.ticker) as (Holding & { quantity: number }) | undefined;
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 h-fit">
          ...
          <div className="flex gap-3 mb-3">
            <div className="flex-1">
              ...
              <input type="number" min="1" step="any" value={shares} onChange={e => setShares(e.target.value)} placeholder="0" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-indigo-500 transition-colors" />
            </div>
            <div className="flex-1">
