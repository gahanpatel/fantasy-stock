  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-slate-400 text-sm mt-1">Sunday, March 29, 2026 · Markets closed</p>
      </div>
          ...
          {LEADERBOARD.slice(0, 7).map((p, i) => {
            ...
            return (
              <div key={p.name} className={`flex items-center gap-3 px-5 py-3 border-t border-slate-100 ${isMe ? 'bg-indigo-50' : 'hover:bg-slate-50'}`}>
                ...
                <span className={`text-sm font-bold ${p.ret >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                  {fmtPct(p.ret)}
                </span>
              </div>
            );
